# Welcome to the Next-gen Machine Emulator (NME)!

This is meant to be a quickstart guide to using NME. More detailed information and background history can be found in the Info page that is installed with NME by using either the standalone Info program or the Info reader in Emacs. 
It can also be found online at [the NME web page](https://phabrics.github.io/tme.html). These documents are generated by the [Texinfo file](https://github.com/phabrics/nme/tme/nme.texi), that is distributed with the NME source tree and will be updated within there going forward.
...
* --enable-threads=`<threading model>` where threading model can be one of sdl, pthreads, or glib. Default threading model if not specified is glib, which is the same as --disable-threads
...
The -m option enables multi-threaded mode in which nmesh will distribute tasks among parallel threads for better efficiency.
...
This is the [nme-make-machs](https://github.com/phabrics/nme/machine/sun/nme-make-machs) script. This script performs 
...
one to use is [project:ttssh2 Tera Term].
